        ...
        li.innerHTML = `
          ...
          <span class="mdc-list-item__text">
              ...
              <span class="mdc-list-item__text__secondary">
                  ${deviceState.temp} C°/${deviceState.tds} ppm/${deviceState.ph} %
              </span>
              <span class="mdc-list-item__text__secondary">
    ...
    try {
      fetch(reportDataUrl)
        ...
        .then(rows => {
          ...
          var maxTdsData = rows.map(row => row.max_tds);
          var avgTdsData = rows.map(row => row.avg_tds);
          var minTdsData = rows.map(row => row.min_tds);

          var maxPhData = rows.map(row => row.max_ph);
          var avgPhData = rows.map(row => row.avg_ph);
          var minPhData = rows.map(row => row.min_ph);
